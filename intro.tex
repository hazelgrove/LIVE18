...
This talk proposal will focus on demonstrating, by example, two examples of live, closure-driven editor services. These two editor services are being integrated into \Hazel, a live functional programming environment being developed by \citet{HazelnutSNAPL}. 

The first editor service, which we detail in Sec.~\ref{sec:examples}, is the \Hazel \emph{live context inspector}, which combines static information with dynamic hole closure information and supports the interactive exploration of recursive hole closures, which arise from incomplete recursive functions. 
...
about the dynamic implications of the choices the palette's user has made. 