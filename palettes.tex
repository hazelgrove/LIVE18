...
%
The implementation of each palette, e.g. \li{grade_cutoffs}, is required to implement a
...
a specified expected type be rendered using the \li{HtmlHole} constructor of the \li{Html} type. When the cursor is in this hole, \Hazel provides all of the usual editor services based on the specified type. Values of the expressions in these holes 
can be referred to, relative to the current user-selected closure, for use by the \li{view} function (here, to generate the orange dots and red warning). 
Each palette must
